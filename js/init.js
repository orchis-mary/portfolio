...
jQuery(document).ready(function(){
	...
	if(document.URL.match("contact")) {
		jQuery('.opening').animate({'z-index':'9999999'},4000);
		jQuery('.drop').animate({'width':'0'},0);
		jQuery('.opening').addClass('loaded');
	}else{
...
function arlo_tm_animate_text(){
	...
	var animateSpan			= jQuery('.arlo_tm_animation_text_word');
		...
		animateSpan.typed({
			strings: ["Nice to Meet you","My name is Mary", "My Job is Web Creator","Active on Twitter as a Custom Caster"],
			loop: true,
			startDelay: 1e3,
			backDelay: 3e3
		});
	var animateSpan			= jQuery('.text-word2');

		animateSpan.typed({
			strings: ["初めまして！","私の名前はマリィ", "Twitterでアイドル活動を行っているWebクリエイターです","カスタムキャスターとして毎日投稿してるから是非見てね♬"],
			loop: true,
			startDelay: 1e3,
			backDelay: 3e3
		});
